...
- **`data/repository`**:
  - `UsageStatsRepositoryImpl` (file `UsageStatsRepository.kt`) — central repository: session/heartbeat writes, daily/weekly aggregation, limit checks, category resolution, hourly buckets, debug info. Bound to `domain.repository.UsageStatsRepository` via `di/RepositoryModule.kt`, but only a subset of its methods are on the interface; tracking/heartbeat/alert-log/debug methods are called on the concrete `Impl` (from `TrackingJob`, `AlertManager`, `AppDetailViewModel`, `DebugViewModel`). `getDailyUsage` drops apps with < 60 s total for the day and computes `totalUsageMs` from the remaining apps only; `getWeeklyUsage` has no such filter.
...
- **`core`** — the app's actual background/business logic, split by concern:
  ...
  - `core/email` — no WorkManager/AlarmManager: `EmailReportSender.checkAndSendIfDue()` is called every minute from `TrackingJob`'s timer, sends when current `HH:mm` equals the configured send time (5-min in-memory dedup), builds a plain-text report and sends it through `EmailSender` (Jakarta Mail/Angus, STARTTLS, one message per recipient). SMTP settings live in Room `email_settings`; defaults (Brevo) are in `BrevoDefaults` in `EmailSettingsViewModel.kt`, the default app password comes from `BuildConfig.SMTP_APP_PASSWORD_DEFAULT` (from `local.properties` → `SMTP_APP_PASSWORD`). Results are written to `report_send_log`.
...
- `TrackingLogStorage` tags and message texts are parsed by `getLogStats()` and the Debug screen (e.g. filtering by `[EmailReport]`, checking for `Service`/`Job`/`Repo`/`Permission`/`UsageStats` tags) — keep them stable when refactoring.
